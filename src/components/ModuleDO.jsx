...
function TabUpload({ project, supabase, onApplyScores }) {
  ...
  async function uploadAndRegister(file) {
    ...
    if (uploadErr && !uploadErr.message?.includes("already exists") && uploadErr.statusCode !== 409) {
      console.warn("Storage DO upload:", uploadErr.message); return null;
    }
    await supabase.from("client_files").insert({
      client_id: project.client_id, project_id: project.id, module_key: "do",
      storage_bucket: "do-documents", storage_path: path,
          ...
          {prop.proposed_commitments?.length > 0 && (
            <div style={{ marginBottom:20 }}>
              ...
              <div style={{ fontSize:12,color:T.t3,marginBottom:10 }}>
                Marca los que quieres crear en el proyecto.
              </div>
                  ...
                  <div key={i} style={{ padding:"12px 14px",background:T.s2,
                    ...
                    <div style={{ display:"flex",alignItems:"flex-start",gap:10 }}>
                      <input type="checkbox" defaultChecked
                        ...
                        style={{ marginTop:3,accentColor:T.do,flexShrink:0 }}/>
...
export default function ModuleDO({ client, supabase }) {
  ...
  return (
    <>
      <style>{CSS}</style>
      <div className="do-module" style={{ padding:"clamp(16px, 4vw, 36px)",maxWidth:1200,overflowX:"hidden" }}>
        {/* THO spectrum stripe */}
        ...
        {/* Línea base */}
        {selProject && !loading && (
          <div style={{ padding:"12px 18px",background:`${T.do}08`,
            border:`1px solid ${T.do}25`,borderRadius:10,marginBottom:20,
            ...
            <button onClick={()=>setShowBaseline(true)} style={{
              padding:"8px 16px",background:T.do,border:"none",borderRadius:8,
